...
3. **Counting stir.** Replace the silent 3-tap gate: a progress ring around the stir button fills 1/3 per tap while Beepo counts "One!/Two!/Three!"; the 3rd triggers `cook()`. Clarifies the step *and* teaches counting. *Animation:* SVG ring via `stroke-dashoffset`; a bigger `rock` per stir. *Sound:* ascending `tone()` per tap. *Hook:* the stir handler + `STIRS_NEEDED`.
